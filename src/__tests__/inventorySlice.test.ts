...
import reducer, { add, remove } from './../redux/slices/inventorySlice';
...
describe('inventorySlice', () => {
	...
	test('Should return the initial state when called without an action', () => {
		expect(reducer(testInventory, { type: '' })).toEqual(testInventory);
	});

	test('When given item data, should add to state', () => {
		expect(reducer({}, add(testInventory.aaaa))).toEqual(testInventory);
	});
